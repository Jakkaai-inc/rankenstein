...
export function renderPreview(
  ...
  if (run.result.kind === 'article') return renderArticlePreview(run, opts);
  const r = run.result;
...
/** Article-mode preview: angle, keyword map, outline, the cited article, a
 *  citation-verdict table, meta + JSON-LD, and guardrail flags. */
function renderArticlePreview(
  run: EngineRunResult,
  opts: { storeDomain?: string },
): string {
  const r = run.result;
  const sel = run.selection;
  const statusOk = r.status === 'pending_review';

  const angleBlock = run.angle
    ? `<h2>1. Angle</h2><p><b>Chosen (${escapeHtml(run.angle.chosen.lens)}):</b> ${escapeHtml(run.angle.chosen.headline)}</p>` +
      `<p class="small">${escapeHtml(run.angle.why)}</p>` +
      `<ul class="small">${run.angle.angles.map((a) => `<li><b>${escapeHtml(a.lens)}:</b> ${escapeHtml(a.headline)}</li>`).join('')}</ul>`
    : '';

  const keywordTable = sel
    ? `<h2>2. Keyword map</h2><table><thead><tr><th>Keyword</th><th class="num">Vol/mo</th><th class="num">KD</th><th>Intent</th><th>Role</th><th>Winnable</th></tr></thead><tbody>${kwRows([sel.primary, ...sel.secondaries])}</tbody></table>`
    : '<p class="small">No keyword selection (self-flagged before select).</p>';

  const outlineBlock = run.outline
    ? `<h2>3. Outline (critic-approved)</h2><ul>${run.outline.sections.map((s) => `<li><b>${escapeHtml(s.h2)}</b>: ${escapeHtml(s.reason)}</li>`).join('')}</ul>`
    : '';

  const afterCard = r.html
    ? `<div class="card after"><span class="tag">Draft (grounded + cited)</span>${r.html}<p class="small"><b>Every external claim is inline-cited and verified.</b> Internal facts trace to the brand catalog.</p></div>`
    : `<div class="card after"><span class="tag">Draft</span><p class="small">No draft produced - self-flagged: ${escapeHtml(run.haltReason ?? 'see flags')}.</p></div>`;

  const citationRows = (run.citations ?? [])
    .map(
      (v) =>
        `<tr><td><a href="${escapeHtml(v.citation.url)}">${escapeHtml(v.citation.anchor)}</a></td>` +
        `<td>${escapeHtml(v.citation.claim)}</td>` +
        `<td>${v.loads ? 'OK' : 'FAIL'}</td><td>${v.supportsClaim ? 'OK' : 'FAIL'}</td><td>${v.authorityOk ? 'OK' : 'FAIL'}</td></tr>`,
    )
    .join('');
  const citationTable = citationRows
    ? `<h2>5. Citation checks</h2><table><thead><tr><th>Source</th><th>Claim</th><th>Loads</th><th>Supports</th><th>Authority</th></tr></thead><tbody>${citationRows}</tbody></table>`
    : '';
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Rankenstein Article Preview - ${escapeHtml(r.title)}</title><style>${STYLE}</style></head>
<body><div class="wrap">
<div style="font-size:26px;font-weight:700;margin:0 0 4px">Article Preview</div>
<p class="sub">${escapeHtml(r.title)} &middot; ${escapeHtml(run.ground.product.vendor)}</p>
<div class="meta">
<span class="chip">Store: ${escapeHtml(opts.storeDomain ?? run.ground.store.primaryDomain ?? '')}</span>
<span class="chip">Primary: ${escapeHtml(r.primaryKeyword)}</span>
<span class="chip">${r.brief.wordCount} words</span>
<span class="chip">Status: <span class="status ${statusOk ? 'ok' : 'flag'}">${escapeHtml(r.status)}</span></span>
<span class="chip">Verifier: ${escapeHtml(r.verdict.verdict)} (${r.verdict.isSelfCheck ? 'self-check' : 'independent'})</span>
</div>
${angleBlock}
...
${outlineBlock}
<h2>4. Draft</h2>
<div class="grid"><div></div>${afterCard}</div>
${citationTable}
<h2>6. Meta + structured data</h2>
<table>
<tr><td style="width:34%;font-weight:600">Title (${r.metaTitle.length} chars)</td><td>${escapeHtml(r.metaTitle)}</td></tr>
<tr><td style="font-weight:600">Meta description (${r.metaDescription.length} chars)</td><td>${escapeHtml(r.metaDescription)}</td></tr>
<tr><td style="font-weight:600">Slug</td><td>${escapeHtml(r.slug)}</td></tr>
</table>
<h3>JSON-LD (Article + FAQPage)</h3>
<pre>${jsonldPre}</pre>
<h2>7. Guardrail flags</h2>
${flags || '<p class="small">No flags.</p>'}
<p class="small" style="margin-top:30px">Generated by the Rankenstein engine (Lane C) &middot; history: ${escapeHtml(r.brief.historyDecision)} &middot; preview only.</p>
</div></body></html>`;
}
