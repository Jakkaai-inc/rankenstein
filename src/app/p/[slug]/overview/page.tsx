...
export default async function OverviewPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!project) notFound();
  const full = await prisma.project.findUnique({ where: { id: project.id }, include: { brandProfile: true, runs: { orderBy: { createdAt: "desc" }, take: 5 } } });

  const grouped = await prisma.contentItem.groupBy({ by: ["status"], where: { projectId: project.id }, _count: true });
  const count = (s: string) => grouped.find((g) => g.status === s)?._count ?? 0;
  const products = await prisma.page.count({ where: { projectId: project.id, type: "PRODUCT" } });
  ...
  const brandConfirmed = full?.brandProfile?.confirmed ?? false;
  const ready = connected && brandConfirmed;

  const publishedItems = await prisma.contentItem.findMany({
    where: { projectId: project.id, status: "PUBLISHED", publishedUrl: { not: null } },
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {!ready && (
        <Card className="border-amber-300 bg-amber-50">
          <CardContent className="text-sm text-amber-900">
            ...
            <Link href={`${base}/settings`} className="font-semibold underline">Go to Settings →</Link>
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <Card className="gap-0 py-4"><CardContent className="px-4">
          ...
          <div className="text-muted-foreground truncate text-xs">{full?.brandProfile?.brandName ?? "—"}</div>
        </CardContent></Card>
      ...
      <Card className="gap-0 py-0">
        <CardHeader className="flex-row items-center justify-between border-b py-3">
          ...
          <Link href={`${base}/products`} className="text-primary text-sm hover:underline">All products →</Link>
        </CardHeader>
        ...
        <Card className="gap-0 py-0">
          ...
          <CardContent className="px-0">
            <ul className="divide-y text-sm">
              ...
              {(full?.runs ?? []).length === 0 && <li className="text-muted-foreground px-6 py-6">No runs yet.</li>}
            </ul>
          </CardContent>
        </Card>
        <Card className="gap-0 py-0">
