...
> **>> LANE A — DEPLOY-TRIGGER GAP + FYI (D, 2026-06-13 ~21:45).** Heads-up that `git push` to `main` does NOT auto-deploy. The CodeBuild project `rankenstein-web` (us-east-2) takes its source from the **S3 zip** `s3://rankenstein-build-src-ue2/rk-src.zip` and has **no GitHub webhook** (`webhook: null`), so nothing rebuilds until someone runs the documented step: `git archive HEAD | (zip) -> aws s3 cp s3://rankenstein-build-src-ue2/rk-src.zip -> aws codebuild start-build --region us-east-2 --project-name rankenstein-web` (per TERMINAL-HANDOFF.md). My Lane D fixes sat on `main` undeployed for ~25 min for this reason. With Gev's OK I ran that step myself and **deployed current `main` (`569ccaa`) — LIVE on studio.rankenstein.app, App Runner deploy SUCCEEDED 21:43 PDT, service RUNNING.** That deploy includes Lane D `7ccbb0d` (review-canvas fixes: delete-comment button, in-body span highlight, whitespace-tolerant cross-cell anchoring) AND your/Lane C `569ccaa` (brand-fact grounding) + `39f065f`/`b868470`. **ASK:** either add a GitHub->CodeBuild webhook (or a CodePipeline source) so pushes auto-deploy for the demo, or make sure whoever owns the demo runs the archive->S3->start-build step after each merge. NB my first attempt with `--source-version <sha>` FAILED (S3 source rejects a git SHA: `s3:ListBucketVersions` AccessDenied on the codebuild role); the working path is upload-zip-then-plain-start-build.
