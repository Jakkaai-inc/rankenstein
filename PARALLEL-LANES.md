...
- Lane A STATE (handoff 2026-06-13, moving to terminal/API-billing): DONE = app shell + auth (redirect-when-logged-out), crawl->brand-draft->confirm (ask-first gate), run orchestrator (`src/lib/run/orchestrator.ts`: public products.json grounding, product-aware seeds, dedup via sourceRef, spend soft-stop, triage), server-side run route `POST /api/v1/projects/:id/run`, GOAL.md + `scripts/goal-check.ts` grader (currently 2/5), deploy pipeline (CodeBuild->ECR->App Runner, autodeploy on :latest; `git archive HEAD` zip -> s3 rankenstein-build-src-ue2 -> start-build). VALIDATED: live generation 2/2 PASS with real fabric keywords after the seed fix. NEXT for Lane A: (1) rebuild+redeploy so the auth fix + current code is live (goal-check #2 still 500s on old image); (2) scale the batch to fill the review queue (`npx tsx scripts/seed-and-run.ts 10`); (3) integrate Lane D review UI + Lane B publish/rollback; (4) one live publish to ezfabricinc.com + rollback. Infra/secrets: `/Users/gevbalyan/Claude/rankenstein-infra.local.md` (untracked, NOT in repo). RDS SG sg-00071a2609e2ee785 open to 0.0.0.0/0 for the day (teardown after). App: App Runner svc .../rankenstein-web/6e7161d50dc84d9dbe93c6f4666687b5 serving rankenstein.app.
